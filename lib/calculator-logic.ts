/* ================================================================
   ...
   Ставка наценки зависит только от суммы товара (обновлено: −0.2% по всем
   тиерам для конкурентоспособности):
     • ≤ 70 000 ₽          → 3.8% / мес  (было 4.0%)
     • 70 001 – 150 000 ₽  → 3.3% / мес  (было 3.5%)
     • > 150 000 ₽         → 2.8% / мес  (было 3.0%)
   Формула расчёта:
...
export const MARKUP_RATE  = 0.038;
...
// ─── Ставка наценки (по сумме товара) ───────────────────────────

export function getRate(price: number): number {
  if (price > 150_000) return 0.028;
  if (price >  70_000) return 0.033;
  return 0.038;
}
